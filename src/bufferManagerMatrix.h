...
class BufferManagerMatrix{
    ...
    PageMatrix insertIntoPool(string MatrixName, int bi,int bj);

    public:
    
    BufferManagerMatrix();
    PageMatrix getPage(string MatrixName, int bi,int bj);
    void writePage(string MatrixName, int bi, int bj, vector<vector<int>> rows, int rowCount,int colCount);
    void deleteFile(string MatrixName, int bi,int bj);
